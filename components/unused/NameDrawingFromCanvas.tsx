  ...
  const loadFontAndDrawDots = () => {
    if (!font) return;
        ...
        if (!ctx) return;
          ...
          svgElement.setAttribute('width', `${bbox.x2 - bbox.x1}`);
          svgElement.setAttribute('height', `${bbox.y2 - bbox.y1}`);
          ...
          if (!hiddenCtx) return;
          // ctx.font = font
          hiddenCanvas.width = bbox.x2 - bbox.x1;
          hiddenCanvas.height = (bbox.y2 - bbox.y1) * 2;
          // ctx.fillText('Hello World', 50, 50); 
          // const img = new Image();
          // img.src = path;

          const svgString = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">${oneData}</svg>`;


        console.log('oneData', svgString)
...
// img.onload = () => {      
  // hiddenCtx.drawImage(img, 0, 0, 100, 100 ) 
  const canvgInstance = Canvg.fromString(hiddenCtx, oneData);
  canvgInstance.start()
